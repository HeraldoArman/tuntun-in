...
/**
 * Imperative handle exposed via `ref` on a `MapMarker`. Lets a parent (e.g. a
 * sidebar list) open/close the marker's popup programmatically.
 */
export type MapMarkerRef = {
  openPopup: () => void;
  closePopup: () => void;
};

const MapMarker = forwardRef<MapMarkerRef, MapMarkerProps>(function MapMarker({
  longitude,
  latitude,
  children,
  onClick,
  ...
  // Expose imperative open/close for the marker's popup so parents (e.g. a
  // sidebar card list) can drive it. togglePopup opens when closed; closing is
